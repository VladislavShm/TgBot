...
import com.tgbot.property.NftCollectionProperties;
import com.tgbot.service.AmqpSender;
import com.tgbot.service.NftService;
import com.tgbot.service.ton.TonService;
            ...
            if (newOwnerWalletInfo.isWallet()) { // Check if current owner is wallet
                ...
                if (saleData.isSale()) { // If it is a sale contract then determine a real owner
                    if (saleData.getOwner() != null) {
                        newOwner = saleData.getOwner().toString(true, true, true, false);
                        if (!Objects.equals(currentNftOwner, newOwner)) {
                            shouldUpdatePrice = tonService.getWalletInfo(newOwner).isWallet();
                        }
                    } else {
