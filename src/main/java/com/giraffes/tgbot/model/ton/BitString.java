...
public class BitString {
    ...
    public BitString(int length) {
        this.array = new int[Double.valueOf(Math.ceil(length >> 3)).intValue()];
        this.cursor = 0;
        this.length = length;
    }
    ...
    public int getUsedBytes() {
        return Double.valueOf(Math.ceil(length >> 3)).intValue();
    }
    ...
    public void checkRange(int n) {
        if (n > this.length) {
            throw new RuntimeException("BitString overflow");
        }
    }
...
//
     ...
     */
    public  void setTopUppedArray(int [] array, boolean fullfilledBytes) {
